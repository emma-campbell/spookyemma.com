...
tags: []
---
Previously, we talked through the choices behind the tech stack used to build my developer blog. The step towards building a functional blog is wiring up [contentlayer](https://www.contentlayer.dev/docs/getting-started), an SDK for transforming unstructured content into type-safe json data structures. Contentlayer is a breeze to use, but in case you're stuck, here is how I set it up.
...
[Github Flavored Markdown](https://github.github.com/gfm/) is a subset of markdown that Github uses in its readme files. We can enable it in our content using `remark-gfm`. Install it using
...
We can render our posts now using Next.js [Dynamic Routes](https://nextjs.org/docs/routing/dynamic-routes). In the `/pages` folder, create a new folder `/post` and a file `[slug].tsx`. Adding brackets to the file name treats it as a parameter.
...
A nice-to-have for any blog is to be able to have visitors "like" your article, and for you to be able to track analytics. Next up, we will set up a (free) [PlanetScale](https://planetscale.com) database with [Prisma](https://www.prisma.io) schemas and type-safe client generation to work with the database.
