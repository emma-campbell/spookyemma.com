...
description: >-
  Adding metrics to a blog post or article is easily accomplished using
  Planetscale and Prisma, allowing for a great developer experience and better
  type-safety with content
...
Planetscale's documentation is pretty solid, and they have [a quickstart guide](https://planetscale.com/docs/tutorials/planetscale-quick-start-guide#create-a-database "PlanetScale Quick Start Guide"). Follow the instructions for creating a database then [promote the default `main` branch to production](https://planetscale.com/docs/tutorials/planetscale-quick-start-guide#to-promote-a-branch-to-production). Planetscale follows a git-like branching concept, which your development branch intended for applying new schema changes.
...
I keep many of my helper methods in a directory called `/lib`. There, we are going to create a new file called `prisma.ts`. Our goal with this helper is to follow the [best Next.js practices](https://pris.ly/d/help/next-js-best-practices "Best Practices for Prisma with Next.js") according to Prisma.
...
> ✨ **Note**: this next part assumes you followed the [last part of this series](/notebook/blog-content-layer) that set up content using contentlayer
...
We are going to open up `prisma/schema.prisma` and add a table called `Stats`. Eventually, we can split our schemas into separate files, but for the time being they won't be too complex and can stay in the same file.
...
On the `POST`, we will use the prisma client's `upsert` method to handle the request. Basically, we will increment the view field on update, or create a new record with the slug.
...
The `GET` is simpler, we will just find the unique record associated with the slug.
...
We can use the `increment` from `usePostViews` to increment the number of views using the `useEffect`<Sidenote id="1">
`useEffect` can be thought of as `componentDidMount`, `componentDidUpdate` and `componentWillUnmount` combined.
