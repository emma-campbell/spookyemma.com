...
In my time at [Hugo Health](https://hugo.health), when I am not working on data operations and scaling, I have been helping to facilitate a major refactor of our systems. A _huge_ part of this is the way that we manage our packages.
...
Generally, I like to run this command for every "feature" included in the PR (though, we \*generally\* don't include more than one feature per pull request as a general practice). When you're completely done with your changes, just before opening the pull request, run `yarn changeset version`. This command will automatically update the version of the package, and generate the corresponding changelog. It will bump the package to the \*minimum\* version required within all included changeset files.

Finally, using the command `yarn changeset publish` will publish the changes to the registry.

# Planning our Bot

When doing research on how to create this bot, I came across an [open issue](https://github.com/changesets/changesets/issues/294 "Azure DevOps Support — Changesets Issue #294") in the `@changesets/changesets` repository from **March 11, 2020** with activity as recent as 4 days ago (January 12, 2023 at the time of writing this).
...
![](/images/posts/azure-changeset-bot/pr_created.png)
...
Use the install button [here](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurefunctions "Azure Functions — VS Code Marketplace") to install the extension, or type "Azure Functions" into the extension search bar in visual studio code and install from there.
...
![](/images/posts/azure-changeset-bot/workspace.png)
...
![](/images/posts/azure-changeset-bot/vscode_command_prompt.png)
...
The only thing we will need in order to grant it access to your projects is a personal access token, which you can create by following [these instructions](https://learn.microsoft.com/en-us/azure/devops/organizations/accounts/use-personal-access-tokens-to-authenticate?view=azure-devops&tabs=Windows#create-a-pat "Create a Personal Access Token — Azure DevOps Docs").
...
const httpTrigger: AzureFunction = async function (
  ...
  const webApi: WebApi = await utils.getWebApi(`https://dev.azure.com/${organization}`);
};
...
```typescript
import { IGitApi } from "azure-devops-node-api/GitApi";
import { GitChange, GitPullRequest } from "azure-devops-node-api/interfaces/GitInterfaces";
```
...
Because this article is already lengthy and interacting with threads in Azure is well documented, I won't delve into details, but you can view the source code [here](https://github.com/emma-campbell/azure-changeset-bot/blob/main/changeset-check/thread-manager.ts "ThreadManager Source — GitHub").
...
You'll want to create a new utility method that gets an entry from the Changelog. This code is lifted directly from the [Changeset Github Action](https://github.com/changesets/action/blob/main/src/utils.ts "Changeset GitHub Action — utils.ts Source").
...
export async function getChangelogEntry(changelog: string, version: string) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (node.type === "heading") {
      ...
      if (match !== null) {
        let level = ChangesetBumpLevels[match[0] as "major" | "minor" | "patch"];
...
export default class Changebot {
  ...
  private async generateChangelog(): Promise<Result> {
    ...
    const commit: CommitResult = await this.git.commit("chore(changelog): generate changelog");
    const push: PushResult = await this.git.push();

    const pkg = require(join(this.git.dir, "package.json"));
    const changelog = await fs.readFile(join(this.git.dir, "CHANGELOG.md"), "utf-8");
