...
description: >-
  Have you seen the feature on Medium that estimates how much time an article
  will take to read? Let's learn to replicate it in our Next.js and contentlayer
  stack.
...
```tsx
<ReadingTime minutes={post.readingTime?.minutes} words={post.readingTime?.words} />
```
