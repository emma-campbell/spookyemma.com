<script lang="ts">
	...
	const filteredEntries = $derived(
		activeFilter === 'all' ? data.entries : data.entries.filter((e) => e.entry === activeFilter)
	);
...
<svelte:head>
	...
	<meta
		name="description"
		content="All writing — blog posts, experiments, notes, guides, micro entries."
	/>
</svelte:head>
...
<PageShell>
	...
	<div class="page-header-row">
		...
		<div class="page-header-content">
			...
			<p class="page-lede">
				Everything — blog posts, experiments, notes, guides, micro entries. Reverse chronological.
	...
	<div class="filter-row">
		...
		<div class="filter-aside">
			<span class="filter-count"
				>showing {filteredEntries.length} {filteredEntries.length === 1 ? 'entry' : 'entries'}</span
			>
		</div>
	</div>
					...
					{#if entry.tags.length > 0}
						<div class="entry-tags">
							{#each entry.tags as tag}
								<span class="entry-tag" style:--badge-color={ENTRY_TYPES[entry.entry]?.color}
									>{tag}</span
								>
							{/each}
						</div>
...
<style>
	...
	.filter-thinking.active {
		background: rgba(232, 168, 48, 0.08);
	}
	.filter-making {
		color: var(--lavender);
	}
	.filter-making.active {
	...
	.feed-entry:hover {
		background: rgba(232, 168, 48, 0.025);
	}
	...
	.feed-entry:hover .entry-read-link {
		color: var(--amber);
	}

	/* Type color variants */
	.type-log .entry-glyph,
	.type-log .entry-type-label {
		color: var(--sage);
	}
	.type-thinking .entry-glyph,
	.type-thinking .entry-type-label {
		color: var(--amber);
	}
	.type-making .entry-glyph,
	.type-making .entry-type-label {
		color: var(--lavender);
	}
